    ...
    class predabst::imp {
        ...
        struct rule_instance_info {
            ...
            void display(std::ostream& out, rule_info const& info) const {
                out << "      head preds cond vars (" << info.m_func_decl->get_name() << "): " << m_head_pred_cond_vars << "\n";
                for (unsigned i = 0; i < m_body_pred_cond_vars.size(); ++i) {
                    out << "      body pred cond vars " << i << " (" << info.m_func_decl->get_name() << "): " << m_body_pred_cond_vars[i] << "\n";
                }
            }
        };
#else
        struct rule_instance_info {
            ...
            void dealloc_solver() {}
            void display(std::ostream& out, rule_info const& info) const {
                out << "      head preds (" << info.m_func_decl->get_name() << "): " << m_head_preds << "\n";
    ...
    public:
        ...
        lbool query(rule_set& rules) {
            ...
            for (unsigned i = 0; i < m_rel_templates.size(); ++i) {
                m_rule2info.push_back(rule_info(m_rel_templates.get(i).m_func_decl, NULL, i, m));
            }
        ...
        model_ref get_model() const {
            ...
            for (unsigned i = 0; i < m_func_decls.size(); ++i) {
                ...
                if (!m_func_decl2info[fdecl]->m_is_output_predicate) {
                    // query symbols are ignored; other predicate symbols are concretized
...
#define RETURN_CHECK_CANCELLED(result) return m_cancel ? l_undef : result;
        ...
        void instantiate_template_rule(unsigned t_id, unsigned r_id) {
            STRACE("predabst", tout << "Instantiating rule " << r_id << " (template " << t_id << ")\n";);
            rule_instance_info& info = m_rule2info[r_id].m_instance_info;
            info.reset();

            // create grounding substitution
            func_decl* fdecl = m_rel_templates.get(t_id).m_func_decl;
            ...
            expr_ref_vector body = apply_subst(temp_body_terms, temp_subst);
            pre_simplify(body);
#ifdef PREDABST_ASSERT_EXPR_UPFRONT
            for (unsigned i = 0; i < body.size(); ++i) {
                m_stats.m_num_solver_assert_invocations++;
        ...
        bool has_template(func_decl* fdecl) const {
            for (unsigned i = 0; i < m_rel_templates.size(); ++i) {
                if (m_rel_templates[i].m_func_decl == fdecl) {
                    return true;
                }
            }
            return false;
        ...
        void print_initial_state(std::ostream& out) const {
            ...
            for (unsigned i = 0; i < m_rel_templates.size(); ++i) {
                ...
                out << "    " << i << ": " << mk_pp(temp.m_func_decl, m) << "(...) := " << temp.m_body << std::endl;
            }
            out << "=====================================\n";
        }
        ...
        void print_inference_state(std::ostream& out, unsigned refine_count, unsigned infer_count) const {
            ...
            for (obj_map<func_decl, func_decl_info*>::iterator it = m_func_decl2info.begin(),
                ...
                if (it->m_value->m_is_output_predicate) {
                    // Skip query symbols since they will never be reachable.
