...
namespace datalog {
    ...
    private:
        ...
        expr_ref mk_core_tree_wf(expr_ref_vector hargs, unsigned n_id, refine_cand_info& refine_cand_info_set) {
            ...
            while (!todo.empty()) {
                todo_item const& item = todo.back();
                todo.pop_back();
                ...
                rule* r = m_rule2info[node.m_parent_rule].m_rule;
                if (r) {
                    STRACE("predabst", tout << "mk_core_tree_wf: node " << n_id << "; " << "("; print_expr_ref_vector(tout, hargs, false); tout << "): was generated by a rule application\n";);
                    expr_ref_vector rule_subst = get_subst_vect(r, hargs, "s");
                    unsigned usz = r->get_uninterpreted_tail_size();
                    unsigned tsz = r->get_tail_size();
                    ...
                    STRACE("predabst", tout << "mk_core_tree_wf: node " << n_id << "; " << "("; print_expr_ref_vector(tout, hargs, false); tout << "): was generated by a template\n";);
                    expr_ref orig_temp_body(m);
                    expr_ref_vector orig_temp_vars(m);
                    bool found = m_template.get_orig_template(node.m_func_decl, orig_temp_body, orig_temp_vars);
                    CASSERT("predabst", found);
